...
{$IFEND}
  ...
  protected
    ...
    property OrgWndProc: NativeInt read FOrgWndProc;
  public
    constructor Create(AHandle: THandle); virtual;
    Destructor Destroy; override;
...
var
  ...
  ChildHandle: HWND;
begin
  //Addlog(Format('Class Name %s Handle %x Message %s',[SysControl.ControlClassName, SysControl.Handle, WM_To_String(Message.Msg)]));
  //OutputDebugString(PChar(Format('Class Name %s Handle %x',[sClassName, wParam])));
