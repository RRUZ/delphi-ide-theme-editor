...
begin
  ...
  if Silent then
    ShellExecute(0, 'open', PChar(ExtractFilePath(LBinaryPath) + 'Updater.exe'), PChar(Format('"%s" -Silent', [GetModuleLocation])), '',
      SW_SHOWNORMAL)
  else
    ShellExecute(0, 'open', PChar(ExtractFilePath(LBinaryPath) + 'Updater.exe'), PChar(Format('"%s"', [GetModuleLocation])), '',
      SW_SHOWNORMAL);
end;
...
begin
  case WM_Message of
    ...
    $00AE:
      Result := 'WM_NCUAHDRAWCAPTION'; // undocumented
    $00AF:
      Result := 'WM_NCUAHDRAWFRAME'; // undocumented
    ...
    Exit('Unknown(' + IntToHex(WM_Message, 4) + ')');
  end; { Case }
...
begin
  ...
  begin
    ...
    if SHGetFileInfo(PChar(Filename), 0, FileInfo, SizeOf(FileInfo), SHGFI_ICON or IconType) = 0 then
      RaiseLastOSError;
